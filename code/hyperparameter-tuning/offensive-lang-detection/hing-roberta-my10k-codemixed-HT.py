...
class OffensivenessDataset(torch.utils.data.Dataset):
...
class Seq_Classifier_HT:
    ...
    def read_data(self):
        train_set = pd.read_csv('/jmain02/home/J2AD011/hxc16/dxk50-hxc16/workspace/nn007/Datasets/Finalized/Offensiveness/codemixed/CMoff-train.csv')  # Paths to the datasets
        valid_set = pd.read_csv('/jmain02/home/J2AD011/hxc16/dxk50-hxc16/workspace/nn007/Datasets/Finalized/Offensiveness/codemixed/CMoff-val.csv')
        return (train_set, valid_set)

    def process_DS(self):
        train_df, valid_df= self.read_data()

        train_encodings = Seq_Classifier_HT.tokenizer(list(train_df['tweet']), max_length=510, truncation=True, padding=True)
        valid_encodings = Seq_Classifier_HT.tokenizer(list(valid_df['tweet']), max_length=510, truncation=True, padding=True)
        
        train_labels = list(train_df['offensiveness'])
        valid_labels = list(valid_df['offensiveness'])

        train_dataset = OffensivenessDataset(train_encodings, train_labels)
        valid_dataset = OffensivenessDataset(valid_encodings, valid_labels)
    ...
    def fine_tune_model(self):
        ...
        res_df.to_csv('hing-roberta-my10k-codemixed.csv')
